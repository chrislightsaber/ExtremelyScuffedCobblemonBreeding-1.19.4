 ...
 */
public class PokeBreedHandlerFactory implements MenuConstructor {
   ...
   */
  public MenuType createMenu(int syncId, Inventory inv, ServerPlayer player) {
    ...
    // Returns the GUI.
    return new MenuType(MenuType.GENERIC_9x6,
       ...
       */
      @Override
      public void slotClicked(int slotIndex, int button, SlotActionType actionType,
                              ServerPlayer player) {
        // If player cancels.
        if (breedSession.cancelled) {
          //player.sendChatMessage(OutgoingChatMessage.create(PlayerChatMessage(Component.literal(String.valueOf("Breeding has been cancelled.")));
